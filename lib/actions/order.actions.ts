...
export async function createOrder() {
	try {
		...
		return {
			...
			redirectTo: `/user/orders/${insertedOrderId}`,
		};
	} catch (error) {
...
export async function updateOrderToPaid({
	...
	await prisma.$transaction(async tx => {
		...
		for (const item of order.orderitems) {
			if (item.variantId === '') {
				// simple product has a variantId of ''
				...
				// variable product has a variantId and a variant (name: e.g. 'Medium / Red')
				// Update stock for variable product variation
